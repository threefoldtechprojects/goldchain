...
func CreateWalletCmd(cli *clientpkg.CommandLineClient) *clientpkg.WalletCommand {
	...
	var (
		rootCmd = &cobra.Command{
			...
			Run:   clientpkg.Wrap(walletCmd.balanceCmd),
		}
		...
		recoverCmd = &cobra.Command{
			...
			Run:   clientpkg.Wrap(walletCmd.recoverCmd),
		}
		lockCmd = &cobra.Command{
			Use:   "lock",
			Short: "Lock the wallet",
			Long:  "Lock the wallet, preventing further use",
			Run:   clientpkg.Wrap(walletCmd.lockCmd),
		}
	...
	by any of the keys in the wallet.`,
			Run: clientpkg.Wrap(walletCmd.signTxCmd),
		}
		seedsCmd = &cobra.Command{
			...
			Run:   clientpkg.Wrap(walletCmd.seedsCmd),
		}
		registerDataCmd = &cobra.Command{
			...
			Run:   clientpkg.Wrap(walletCmd.registerDataCmd),
		}
	...
	providing a net flow of coins and blockstakes for each transaction.`,
			Run: clientpkg.Wrap(walletCmd.listTransactionsCmd),
		}
		unlockCmd = &cobra.Command{
			Use:   `unlock`,
			Short: "Unlock the wallet",
			Long:  "Decrypt and load the wallet into memory",
			Run:   clientpkg.Wrap(walletCmd.unlockCmd),
		}
		...
		loadSeedCmd = &cobra.Command{
			...
			Run:   clientpkg.Wrap(walletCmd.loadSeedCmd),
		}
	...
	`,
		...
		sendTxCmd = &cobra.Command{
			...
			Run:   clientpkg.Wrap(walletCmd.sendTxCmd),
		}
	...
	// return root command
	return &clientpkg.WalletCommand{
		Command:       rootCmd,
		RootCmdSend:   sendCmd,
		RootCmdLoad:   loadCmd,
		RootCmdList:   listCmd,
...
type walletCmd struct {
	cli          *clientpkg.CommandLineClient
	sendCoinsCfg struct {
		Data             []byte
...
`, encStatus, currencyConvertor.ToCoinStringWithUnit(status.ConfirmedCoinBalance),
		...
		currencyConvertor.ToCoinStringWithUnit(status.ConfirmedCustodyFeeDebt),
