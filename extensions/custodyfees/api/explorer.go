...
func RegisterExplorerCustodyFeesHTTPHandlers(router rapi.Router, cs modules.ConsensusSet, plugin *custodyfees.Plugin) {
	router.GET("/explorer/custodyfees/coinoutput/age/:id", NewCoinOutputGetAgeHandler(cs, plugin))
	router.GET("/explorer/custodyfees/coinoutput/fee/:id", NewCoinOutputGetCustodyFeeHandler(cs, plugin))
}
